...
the engine code (study/, gui/) should need to reach into a plain dict with
...
class MarkupsConfig:
    ...
    color: Optional[List[float]] = None
    size: Optional[float] = None             # point size: Slicer's Markups display 'Scale' (or millimetres if size_absolute); unset -> Slicer's default
    size_absolute: Optional[bool] = None     # True: size is in mm (a real-world size, so a 3D glyph spans several slices) instead of the relative scale
    glyph_type: Optional[str] = None         # e.g. 'Sphere3D' - a vtkMRMLMarkupsDisplayNode glyph type name; unset -> Slicer's default
...
class BatchExportConfig:
    """cfg.batch_export - what BatchProcessor (study/batch_processor.py) does
    ...
    # "{time}"/"{datetime}"/"{index}" - see batch_processor.BatchProcessor._resolve_pattern_path())
...
@dataclass
class BrushConfig:
    """segment_editor.brush - Segment Editor's Paint-effect brush shape/size, applied to the segment editor node's attributes (see utils/segment_editor.py)."""
...
def normalize_output_dir_pattern(value, key_columns):
    """The top-level output_dir_pattern as a format string. Older configs gave a LIST of column names (["ID", "measurement"]) meaning one path part each - accepted and turned into "{ID}/{measurement}". Empty -> the key columns, joined the same way."""
    if isinstance(value, (list, tuple)):
        value = "/".join(f"{{{c}}}" for c in value if c)
    return value or "/".join(f"{{{k}}}" for k in key_columns)


@dataclass
class StudyConfig:
    """The whole parsed config.json. Everything the engine code needs is reachable from here via attributes - no raw dict.get() chains."""
    study_dir: str = "."
    ...
    def from_dict(cls, raw: Optional[dict], base_dir: str = "."):
        ...
        return cls(
            ...
            output_dir_pattern=normalize_output_dir_pattern(raw.get("output_dir_pattern"), key_columns),
